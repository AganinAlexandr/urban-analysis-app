...
def detect_group_from_text(text: str) -> str:
    """
    Определяет группу объекта на основе текста (название + отзывы)
    
    Args:
        text: Объединенный текст для анализа
        
    Returns:
        Определенная группа или 'undetected'
    """
    if not text:
        return 'undetected'
    ...
    group_keywords = {
        'university': [
            'университет', 'институт', 'академия', 'вуз', 'студент', 'преподаватель', 
            'лекция', 'сессия', 'факультет', 'кафедра', 'диплом', 'зачет', 'экзамен',
            'педагогический', 'государственный', 'медицинский', 'технический'
        ],
        'school': [
            'школа', 'лицей', 'гимназия', 'ученик', 'учитель', 'урок', 'класс',
            'начальная', 'средняя', 'старшая', 'общеобразовательная'
        ],
        'hospital': [
            'больница', 'клиника', 'врач', 'медицинский', 'лечение', 'пациент', 
            'прием', 'отделение', 'стационар', 'амбулатория', 'поликлиника'
        ],
        'pharmacy': [
            'аптека', 'лекарство', 'препарат', 'фармацевт', 'медикамент',
            'таблетка', 'сироп', 'мазь', 'рецепт'
        ],
        'kindergarden': [
            'детский сад', 'воспитатель', 'ребенок', 'игра', 'группа',
            'дошкольный', 'ясли', 'детсад'
        ],
        'polyclinic': [
            'поликлиника', 'врач', 'прием', 'консультация', 'амбулатория',
            'медицинский центр', 'диспансер'
        ],
        'shopmall': [
            'торговый центр', 'магазин', 'покупка', 'товар', 'цены',
            'молл', 'торговый дом', 'супермаркет', 'гипермаркет'
        ],
        'resident_complex': [
            'жилой комплекс', 'дом', 'квартира', 'жилье', 'недвижимость',
    ...
    # Выбираем группу с максимальным количеством совпадений
    if group_scores:
        best_group = max(group_scores, key=group_scores.get)
        best_score = group_scores[best_group]
        
        # Если несколько групп имеют одинаковый балл, возвращаем 'undetected'
        tied_groups = [g for g, s in group_scores.items() if s == best_score]
