#!/usr/bin/env python3
"""
Детальный анализ определения группы для конкретного объекта
"""
...
def debug_detection():
    """Детально анализирует определение группы для тестового объекта"""
    
    print("=== ДЕТАЛЬНЫЙ АНАЛИЗ ОПРЕДЕЛЕНИЯ ГРУППЫ ===")
    
    # Тестовый объект
    object_name = "Школа № 1238, дошкольный корпус"
    review_text = "Лучший детский сад на свете! Ольге Сергеевне, Светлане Владимировне, Ирине Владимировне низкий поклон за неравнодушие и за заботу, которую они каждый день дарят ребятам нашей группы. Сын ходит в сад с удовольствием, многому научился благодаря нашим воспитателям. Большое спасибо!"
        ...
        for keyword_type, keyword, weight in keywords_list:
            ...
            # Проверяем извлеченные ключевые слова
                ...
                if normalized_keyword in normalized_name:
                    ...
                    matches.append(f"✅ '{keyword}' в названии (извлеченное, +{weight*0.5})")
                if normalized_keyword in normalized_review:
                    text_score += weight
                    matches.append(f"✅ '{keyword}' в отзыве (извлеченное, +{weight})")
