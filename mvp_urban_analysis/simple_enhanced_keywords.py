#!/usr/bin/env python3
"""
Упрощенная система ключевых слов с базовой нормализацией
"""
import sqlite3
import os
import re
...
class SimpleKeywordProcessor:
    """Упрощенный процессор ключевых слов с базовой нормализацией"""
...
def create_simple_enhanced_keywords_table():
    """Создает упрощенную улучшенную таблицу ключевых слов"""
    ...
    try:
        ...
        # Создаем упрощенную улучшенную таблицу ключевых слов
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS simple_enhanced_keywords (
        ...
        cursor.execute("DELETE FROM simple_enhanced_keywords")
        
        conn.commit()
        conn.close()
        
        print("✅ Упрощенная улучшенная таблица ключевых слов создана")
        return True
...
def analyze_simple_enhanced_keywords():
    """Анализирует ключевые слова с упрощенной нормализацией"""
    print("🔍 АНАЛИЗ УПРОЩЕННЫХ УЛУЧШЕННЫХ КЛЮЧЕВЫХ СЛОВ")
    print("=" * 50)
    
    processor = SimpleKeywordProcessor()
    db_path = 'urban_analysis_fixed.db'
...
def find_simple_enhanced_unique_keywords(group_words, all_words, min_frequency=2):
    """Находит уникальные ключевые слова с упрощенной нормализацией"""
    print("\n🎯 ПОИСК УНИКАЛЬНЫХ КЛЮЧЕВЫХ СЛОВ (С УПРОЩЕННОЙ НОРМАЛИЗАЦИЕЙ)")
    print("=" * 50)
...
def save_simple_enhanced_keywords_to_db(unique_keywords):
    """Сохраняет упрощенные улучшенные ключевые слова в БД"""
    db_path = 'urban_analysis_fixed.db'
    ...
    try:
        ...
        for group_type, words in unique_keywords.items():
            for word, count in words:
                ...
                cursor.execute("""
                    INSERT INTO simple_enhanced_keywords 
        ...
        print(f"\n✅ Сохранено упрощенных улучшенных ключевых слов: {added_count}")
        return True
...
def detect_group_by_simple_enhanced_keywords(object_name, review_text):
    """Определяет группу объекта с упрощенным улучшенным алгоритмом"""
    db_path = 'urban_analysis_fixed.db'
...
def test_simple_enhanced_system():
    """Тестирует упрощенную улучшенную систему"""
    print("=== ТЕСТИРОВАНИЕ УПРОЩЕННОЙ УЛУЧШЕННОЙ СИСТЕМЫ ===")
    ...
    for i, (object_name, review_text) in enumerate(test_cases, 1):
        ...
        detected_group, confidence = detect_group_by_simple_enhanced_keywords(object_name, review_text)
...
def main():
    """Основная функция"""
    print("=== УПРОЩЕННАЯ УЛУЧШЕННАЯ СИСТЕМА КЛЮЧЕВЫХ СЛОВ ===")
    print(f"⏰ Время: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    # Создаем упрощенную улучшенную таблицу
    print("🔧 Создание упрощенной улучшенной таблицы ключевых слов...")
    if not create_simple_enhanced_keywords_table():
        print("❌ Ошибка создания таблицы")
        return
    
    # Анализируем данные
    group_words, all_words = analyze_simple_enhanced_keywords()
    ...
    unique_keywords = find_simple_enhanced_unique_keywords(group_words, all_words, min_frequency=2)
    ...
    if save_simple_enhanced_keywords_to_db(unique_keywords):
        print("✅ Упрощенные улучшенные ключевые слова сохранены в БД")
    else:
        print("❌ Ошибка сохранения")
    
    # Тестируем систему
    print("\n🧪 Тестирование упрощенной улучшенной системы...")
    test_simple_enhanced_system()
    
    print("\n🎉 Упрощенная улучшенная система ключевых слов настроена!")
