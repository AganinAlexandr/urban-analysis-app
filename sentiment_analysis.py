...
def plot_correlation_matrix(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig('diagrams/correlation_matrix.png')
    plt.close()
...
def plot_distributions(df: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig('diagrams/sentiment_distribution.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('diagrams/review_type_distribution.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('diagrams/response_type_distribution.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('diagrams/combined_distribution.png')
    plt.close()
...
def plot_review_parts_analysis(df: pd.DataFrame):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('diagrams/review_parts_ratio.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('diagrams/review_parts_correlation.png')
    plt.close()
...
def plot_feature_analysis(df: pd.DataFrame, feature_name: str, base_value: float = None, 
    ...
    if feature_name == 'rating':
        ...
        ax2.plot(values, cumulative_percentages, 'r-', linewidth=2, label='Накопленный процент')
        ax2.set_ylabel('Накопленный процент (%)', color='red')
        ax2.tick_params(axis='y', labelcolor='red')
        
        plt.title('Анализ распределения оценок отзывов')
        plt.legend(loc='upper left')
        
    else:
        # Общая обработка для других признаков
        if base_value is None:
            if feature_name == 'min_words_threshold':
                base_value = 30
            elif feature_name == 'min_positive_words':
                base_value = 1
            elif feature_name == 'min_negative_words':
                base_value = 1
            elif feature_name == 'complex_threshold':
                base_value = 0.05
        ...
        # Генерируем значения для анализа
        values = []
        for i in range(-num_steps, num_steps + 1):
            values.append(base_value + i * step)
        
        # Анализируем каждый параметр
        results = []
        for value in values:
            # Здесь должна быть логика анализа с разными параметрами
            # Пока используем простое моделирование
            if feature_name == 'min_words_threshold':
                # Симулируем количество сложных отзывов при разных порогах
                complex_count = len(df[df['review_text'].str.split().str.len() >= value])
                results.append(complex_count)
            else:
                # Для других параметров используем случайные значения
                import random
                results.append(random.randint(50, 200))
        
        # Построение графика
        ax1.plot(values, results, 'b-', linewidth=2, marker='o')
        ax1.set_xlabel(f'Значение параметра {feature_name}')
        ax1.set_ylabel('Количество сложных отзывов', color='blue')
        ax1.tick_params(axis='y', labelcolor='blue')
        
        plt.title(f'Анализ влияния параметра {feature_name}')
    
    plt.tight_layout()
    plt.savefig(f'diagrams/feature_analysis_{feature_name}.png')
    plt.close()
...
def create_3d_visualization(df: pd.DataFrame):
    ...
    fig1.write_html('diagrams/3d_analysis_words_threshold.html')
    ...
    fig2.write_html('diagrams/3d_analysis_ratings.html')
